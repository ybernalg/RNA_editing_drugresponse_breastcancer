...
data <- read.delim2("~/Desktop/PAPER1/antra/input/CCLE24_master_table_Anthracyclines_groupCounts.tsv")


library(dplyr)

#> colnames(data)
#[1] "Region"                                 "Position"                               "Reference"                             
#[4] "bam.rc.A.C.G.T._G27249.AU565.1_HS"      "bam.rc.A.C.G.T._G30590.BT.549.1_HS"     "bam.rc.A.C.G.T._G27365.CAL.148.1_HS"   
#[7] "bam.rc.A.C.G.T._G26176.CAL.51.2_HS"     "bam.rc.A.C.G.T._G27334.CAMA.1.1_HS"     "bam.rc.A.C.G.T._G28036.JIMT.1.1_HS"    
#[10] "bam.rc.A.C.G.T._G28064.MDA.MB.468.1_HS" "bam.rc.A.C.G.T._G26250.BT.474.2_LS"     "bam.rc.A.C.G.T._G27300.BT.483.1_LS"    
#[13] "bam.rc.A.C.G.T._G27355.EFM.192A.1_LS"   "bam.rc.A.C.G.T._G41743.HCC1419.5_LS"    "bam.rc.A.C.G.T._G27310.HCC1428.1_LS"   
#[16] "bam.rc.A.C.G.T._G20496.HCC1954.2_LS"    "bam.rc.A.C.G.T._G28898.HCC202.3_LS"     "bam.rc.A.C.G.T._G27278.ZR.75.30.1_LS"  
#[19] "HS_NAcount"                             "HS_count"                               "LS_NAcount"                            
#[22] "LS_count"  


##### preparación data ###########

data <- filter(data, HS_count > 3 & LS_count >4)
...
colnames(data) <- c("Uploaded_variation","ensembl",
                    "AU565", "BT549", "CAL148", "CAL51",
                    "CAMA1", "JIMT1",  "MDAMB468", 
                    "BT474", "BT483","EFM192A", 
                    "HCC1419", "HCC1428", "HCC1954", "HCC202", "ZR7530")
...
library("stringr")
data[ , 18:21] <- str_split_fixed(data$AU565, ",", 4)
data[ , 22:25] <- str_split_fixed(data$BT549, ",", 4)
data[ , 26:29] <- str_split_fixed(data$CAL148, ",", 4)
data[ , 30:33] <- str_split_fixed(data$CAL51, ",", 4)
data[ , 34:37] <- str_split_fixed(data$CAMA1, ",", 4)
data[ , 38:41] <- str_split_fixed(data$JIMT1, ",", 4)
data[ , 42:45] <- str_split_fixed(data$MDAMB468, ",", 4)
data[ , 46:49] <- str_split_fixed(data$BT474, ",", 4)
data[ , 50:53] <- str_split_fixed(data$BT483, ",", 4)
data[ , 54:57] <- str_split_fixed(data$EFM192A, ",", 4)
data[ , 58:61] <- str_split_fixed(data$HCC1419, ",", 4)
data[ , 62:65] <- str_split_fixed(data$HCC1428, ",", 4)
data[ , 66:69] <- str_split_fixed(data$HCC1954, ",", 4)
data[ , 70:73] <- str_split_fixed(data$HCC202, ",", 4)
data[ , 74:77] <- str_split_fixed(data$ZR7530, ",", 4)


data <- subset(data, select = -c(3:17))
...
colnames(data) <- c("Uploaded_variation", "strand", 
                    ...
                    "JIMT1_A", "JIMT1_C","JIMT1_G", "JIMT1_T",
                    ...
                    "HCC202_A",  "HCC202_C", "HCC202_G", "HCC202_T",
...
data$CAMA1_ref = ifelse(data$strand == "A/G", data$CAMA1_A, data$CAMA1_T)
data$CAMA1_alt = ifelse(data$strand == "A/G", data$CAMA1_G, data$CAMA1_C)

data$JIMT1_ref = ifelse(data$strand == "A/G", data$JIMT1_A, data$JIMT1_T)
data$JIMT1_alt = ifelse(data$strand == "A/G", data$JIMT1_G, data$JIMT1_C)
...
data$HCC202_ref = ifelse(data$strand == "A/G", data$HCC202_A, data$HCC202_T)
data$HCC202_alt = ifelse(data$strand == "A/G", data$HCC202_G, data$HCC202_C)
...
data <- subset(data, select = -c(3:38))
data <- subset(data, select = -c(3:26))




############### Statistical test REDITs ############################


data[data == ""] <- NA
Low_sensitivity <-  c("BT474", "BT483", "EFM192A", "HCC1419", "HCC1428",
                      "HCC1954",  "HCC202","ZR7530")
...
create.matrix <- function(site){
  ...
  colnames(mat.editions) <- c( "AU565" ,"BT549","CAL148" ,"CAL51" , "CAMA1",
                               "JIMT1", "MDAMB468","BT474","BT483","EFM192A",
                               "HCC1419","HCC1428","HCC1954","HCC202","ZR7530")
...
for(i in 1:nrow(data)){
  mat.ed <- create.matrix(data[i,3:32])
...
data1 <- filter(data, differential != "non_differential")
